<x-modal name="edit-item-{{ $data->id }}">
    <div class="relative p-4 sm:p-5 w-full max-w-2xl h-full md:h-auto">
        ...
        <div class="relative h-full pb-4">
            ...
            <!-- Modal body -->
            <form action="{{ route('task.update', $data->id) }}" method="post" id="additemform">
                @csrf
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div>
                        <x-input-label for="title" class="mb-2">Title</x-input-label>
                        <x-text-input type="text" name="title" id="title" class="p-2.5"
                            placeholder="Title Task" required="" value="{{ $data->title }}" />
                    </div>
                    <div>
                        <x-input-label for="category" class="mb-2">Status</x-input-label>
                        <select id="category" name="status"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 pl-4 pr-8 dark:bg-darker dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option class="py-2" value="existing" {{ $data->status == 'unfinished' ? 'selected' : '' }}>
                                Unfinished</option>
                            <option class="py-2" value="progressing"
                                {{ $data->status == 'progressing' ? 'selected' : '' }}>Progressing</option>
                            <option class="py-2" value="complete" {{ $data->status == 'complete' ? 'selected' : '' }}>
                                Complete</option>
                        </select>
                    </div>
                </div>
                <div class="grid">
                    <div>
                        ...
                        <textarea name="description" id="description"
                            ...
                            placeholder="description task" required value={{ $data->description }}>{{ $data->description }}</textarea>
                    </div>
                </div>
